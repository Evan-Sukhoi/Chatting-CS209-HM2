    ...
    try {
      switch (msg.getDataType()) {
        ...
        case DataType.MESSAGE_CLIENT_EXIT:
          ServerService.sendOfflineMsg(user.getUserID());
        ...
        case DataType.MESSAGE_IMAGE_MESSAGE:
          //准备回复
          System.out.println(
              "服务器收到来自客户【" + user.getUserID() + "】的文件/图片消息: " + msg.getFileName());
          if (ServerService.chatList.get(msg.getChatID()).getChatType().equals("private")) {
            System.out.println("服务器将客户【" + user.getUserID() + "】的私聊文件/图片消息发送给客户【"
                + msg.getSendTo() + "】");
            ServerService.sendPrvMsg(msg);
          } else if (ServerService.chatList.get(msg.getChatID()).getChatType().equals("group")) {
            System.out.println("服务器将客户【" + user.getUserID() + "】的群聊文件/图片消息发送群聊【"
                + msg.getSendTo() + "】");
            ServerService.sendGrpMsg(msg);
          }
          break;
        case DataType.MESSAGE_PING:
          ServerService.sendPingRtn(msg);
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
